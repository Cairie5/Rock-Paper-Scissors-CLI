...
from models import Player, Game, engine
...
def main():
    ...
    while True:
        player_choice = input("Enter your choice (rock/paper/scissors/history/q to quit): ").lower()

        if player_choice == 'q':
            break
        elif player_choice == 'history':
            games = get_game_history(args.player_name)
            print("\nGame History:")
            for game in games:
                print(f"Timestamp: {game.timestamp}, Result: {game.result}")
            print(f"Total Wins: {get_player_wins(args.player_name)}")
        elif player_choice not in ['rock', 'paper', 'scissors']:
            print("Invalid choice. Please choose rock, paper, scissors, history, or q to quit.")
        else:
